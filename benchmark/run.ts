...
function isMatch(resultPath: string, truthPath: string): boolean {
  ...
  if (normResult.includes(normTruth) || normTruth.includes(normResult)) return true;
...
function computeRecall(results: SearchResult[], groundTruth: string[], k: number): number {
  const topK = results.slice(0, k);
  const found = groundTruth.filter((gt) => topK.some((r) => isMatch(r.file_path, gt)));
  return found.length / groundTruth.length;
}

function computePrecision(results: SearchResult[], groundTruth: string[], k: number): number {
  const topK = results.slice(0, k);
  if (topK.length === 0) return 0;
  const relevant = topK.filter((r) => groundTruth.some((gt) => isMatch(r.file_path, gt)));
...
function checkContentHit(results: SearchResult[], expectedContent?: string): boolean | null {
...
async function main() {
  ...
  for (const q of queries) {
    ...
    if (verbose) {
      ...
      if (verbose && recall5 === 0 && searchResults.length > 0) {
        console.log(`     Got: ${searchResults.slice(0, 3).map((r) => r.file_path).join(", ")}`);
  ...
  const corpusFiles = parseInt(
    execSync(`find benchmark/corpus -name "*.md" | wc -l`, { encoding: "utf-8" }).trim()
  );
  ...
  await writeFile(resolve(RESULTS_DIR, resultsFile), JSON.stringify(summary, null, 2));
